...
exports.homePage = async (req, res, next) => {
    const [row] = await dbConfig.execute("SELECT * FROM `users` WHERE `id`=?", [req.session.userID]);
...
exports.register = async (req, res, next) => {
    ...
    try {
        ...
        const [row] = await dbConfig.execute(
...
exports.login = async (req, res, next) => {
    ...
    try {
        ...
        const [row] = await dbConfig.execute('SELECT * FROM `users` WHERE `email`=?', [body._email]);
